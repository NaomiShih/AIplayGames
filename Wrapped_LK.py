...
class player:
    ...
    #新增player死亡定義            
    def player_dead(player, terminal):
        terminal = False
        if player1.Y > 640:
...
class Platform:
    def __init__(self, X, Y, dmg, img,reward):
        self.X = X
        self.Y = Y
        self.dmg = dmg
        self.img = img
...
Board1  = Platform(240, 600, -1, KidImg.Board, 0)
Nails1   = Platform(Xlist[4], 1241, 3,KidImg.Nails, 0)
Nails2   = Platform(Xlist[2], 1241, 3,KidImg.Nails, 0)
Nails3   = Platform(Xlist[7], 1241, 3,KidImg.Nails, 0)
Nails4   = Platform(Xlist[5], 1241, 3,KidImg.Nails, 0)
Board2  = Platform(Xlist[3], 1241, -1, KidImg.Board, 0)
Board3  = Platform(Xlist[3], 1241, -1, KidImg.Board, 0)
Trampoline  = Platform(Xlist[6], 1241, -1, KidImg.TrampolineUP, 0)
ceil = Platform(20, 80, 0,  KidImg.ceil, 0)
conveyor_left = Platform(Xlist[5], 1241, -1,  KidImg.conveyor_left, 0)
conveyor_right = Platform(Xlist[6], 1241, -1,  KidImg.conveyor_right, 0)
...
class GameState(player, Platform):
    ...
    def frame_step(reward, terminal, image_data):
        pygame.event.pump()
        ...
        if input_actions[1] == 1:
            stairs_showed = Platform.Platform_Rect(Platform, Player, PlatWidth, PlatHigth, PlayerWidth, PlayerHight, CountF)
            player.X -= player.vel
        
        # input_actions[2] == 1: turn right
        if input_actions[2] == 1:
            stairs_showed = Platform.Platform_Rect(Platform, Player, PlatWidth, PlatHigth, PlayerWidth, PlayerHight, CountF)
            player.X += player.vel
            
                    
        #除了platform中定義的，剩下兩個未定義
        ...
        if player.player_dead(player, True):
            reward = -5
            return reward, terminal
        ...
        FPSCLOCK.tick(FPS)
        return reward, terminal, image_data
...
while run:
    ...
    if times < 3 and player.player_dead(player,False):
        CountF += 1
    
    
    elif player.player_dead(player,True):
         run = False
