...
HPJudge = [True]
keys =  [False, False, False]#設定上下左右判定的list
CountF = 0#設定往下樓層的變數
...
while run:
    ...
    if times < 3 and player1.Y <= 640:
        CountF += 1
         ...
         #playsound('/Users/huangyuting/Desktop/AIplayGames-main/LiitleKidsNew4/assets/sounds/Fall 2.mp3', block=True)
         run = False
    ...
    if player1.HP == 10:
        ...
        #playsound('/Users/huangyuting/Desktop/AIplayGames-main/LiitleKidsNew4/assets/sounds/Stabbed Scream.mp3', block=True)
        run = False
...
class GameState(Platform,player):
    
    
    def __init__(self):
        #super().__init__(self,X, Y,img,HP, vel)
        #super(player,self).__init__(self, X, Y, HP, img, vel)
        #super(Platform,self).__init__(self, X, Y,  img)
        self.playerx = int(240)#agent initial position
        self.playery = int(120)#agent initial
        self.image=pygame.image.load('./assets/playerNomal.png')#主角
        self.rect = self.image.get_rect()#主角
        self.ceiling=pygame.image.load('./assets/ceiling.png')#天花板
        self.ceil_rect=self.ceiling.get_rect()
        self.Board = pygame.image.load('./assets/normal.png')#板子
        self.Board_rect=self.Board.get_rect()#板子
        self.Nails = pygame.image.load('./assets/nails.png')#尖刺
        self.Nails_rect=self.Nails.get_rect()#尖刺
        self.wall =  pygame.image.load('./assets/wall.png')#牆壁
        self.wall_rect=self.wall.get_rect()#牆壁
        self.TrampolineUP = pygame.image.load('./assets/trampolineUP.png')#彈簧
        self.TrampolineUP_rect=self.TrampolineUP.get_rect()
        self.TrampolineDown = pygame.image.load('./assets/trampolineDown.png')#彈簧
        self.conveyor_left = pygame.image.load('./assets/conveyor_left1.png')#左傳輸
        self.conveyor_right = pygame.image.load('./assets/conveyor_right1.png')#右傳輸
    ...
    def frame_step(self, keys):
        ...
        if keys[0]==False and keys[1]==True and keys[2]==False:
            self.rect.left -= self.vel
            
        elif keys[0]==False and keys[1]==False and keys[2]==True:
        ...
        #判斷邊界
        if (pygame.Rect.colliderect(self.rect, self.wall_rect)):
            player1.X  = 18
        elif (pygame.Rect.colliderect(self.rect, self.wall_rect)):
            player1.X  = 419
        
        #曾數計算
        if times < 3 and self.playery <= 640:
            self.CountF += 1
        
        
        
        #判斷碰撞
        if (pygame.Rect.colliderect(self.rect, self.ceil_rect ) == 1) and self.ceiling == KidImg['ceil'] : 
            self.score-=1
            self.HP-=3
            reward = -1
        
        if(pygame.Rect.colliderect(self.rect, Board1Rect ) == 1) and (self.rect.y+31)>Platform.Y:
            if (self.image.img == KidImg['Normal']):
                self.score+=1
                self.HP+=1
                reward = 1
                
        if(pygame.Rect.colliderect(self.rect,Nails1Rect ) == 1) and (self.rect.y+31)>Platform.Y:
            if (self.Nails.img == KidImg['Nails']):
                self.score-=1
                self.HP+=1
                reward = -1
        
        if(pygame.Rect.colliderect(self.rect,TrampolineRect) == 1) and (self.rect.y+31)>Platform.Y:
            if (self.TrampolineUP.img == KidImg['TrampolineUP']):
                self.score+=0.1
                self.HP+=1
                reward = 0.1
        
        if(pygame.Rect.colliderect(self.rect, conveyor_rightRect ) == 1) and (self.rect.y+31)>Platform.Y:
            if(self.conveyor_left.img == KidImg['conveyor_right']):
                self.score+=0.1
                self.HP+=1
                reward = 0.1
        
        if(pygame.Rect.colliderect(self.rect, conveyor_leftRect) == 1) and (self.rect.y+31)>Platform.Y:
            if(self.conveyor_right.img == KidImg['conveyor_left']):   
                self.score+=0.1
                self.HP+=1
                reward = 0.1
        #判斷死亡
        ...
        for x in range (80, 700, 100):#設定牆壁 以及生命值
            ...
            screen.blit(self.img, (240,120))#設定玩家
            ...
            head_font = pygame.font.SysFont(None, 60)#設定大小60的標題框框
            text_surface = head_font.render('B%04dF'%self.CountF, True, (121, 255, 121))    
